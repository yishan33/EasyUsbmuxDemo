...
+ (dispatch_io_t)connect_to_usbmuxd_channel {
    int fd = [self connect_to_usbmuxd];
    err_handler err_handler = ^(int error) {
        printf("Error is: %d\n", error);
    };
...
+ (void)send_packet:(NSDictionary *)packetDict tag:(int)tag channel:(dispatch_io_t)channel {
...
+ (void)send_msg:(NSString *)msg {
    dispatch_data_t payload = [self PTExampleTextDispatchDataWithString:msg];
    dispatch_data_t frame = [self createDispatchDataWithFrameOfType:101 frameTag:0 payload:payload];
